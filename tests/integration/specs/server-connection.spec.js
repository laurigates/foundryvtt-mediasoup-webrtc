...
test.describe('MediaSoup Server Connection', () => {
    ...
    test('should test WebSocket connection to localhost', async () => {
        ...
        expect(status).toMatch(
            /(Connection test passed|Connection timeout|Connection test failed)/
        );
    });
    ...
    test('should handle invalid server URL gracefully', async () => {
        ...
        await page.evaluate(() => {
            window.game.settings.set(
                'mediasoup-vtt',
                'mediaSoupServerUrl',
                'ws://invalid-server-that-does-not-exist.local:9999'
            );
        });
    ...
    test('should validate server URL before connection', async () => {
        const _client = await page.evaluate(() => window.MediaSoupVTT_Client);
