...
async function waitForClientWithRetry(page, options = {}) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            const currentPolling = basePolling * 1.5 ** attempt;
            const attemptTimeout = Math.min((timeout / maxRetries) * (attempt + 2), timeout);

            console.log(
                `[Retry ${attempt + 1}/${maxRetries}] Waiting for client with timeout: ${attemptTimeout}ms, polling: ${currentPolling}ms`
            );
            ...
            const result = await page.waitForFunction(
                () => {
                    ...
                    const isCorrectType =
                        hasClient &&
                        window.MediaSoupVTT_Client.constructor.name === 'MediaSoupVTTClient';
                    const hasRequiredMethods =
                        hasClient &&
                        typeof window.MediaSoupVTT_Client.updateServerUrl === 'function';
                    ...
                    if (
                        ...
                        window.location.search.includes('ci-debug')
                    ) {
                        if (!hasClient && window.testSandbox) {
                            ...
                            console.log(
                                `[CI-Debug] Client check attempt ${attempt + 1}:`,
                                debugInfo
                            );
                        }
                    }
            ...
            if (isBrowserClosed) {
                console.error(
                    `[Browser Closed] Browser crashed during attempt ${attempt + 1}. Error: ${error.message}`
                );
                throw new Error(
                    `Browser crashed during waitForClientWithRetry. This suggests browser instability in CI.`
                );
            }
            ...
            if (attempt === maxRetries - 1) {
                ...
                try {
                    if (!page.isClosed()) {
                        debugInfo = await page.evaluate(() => ({
                            ...
                            windowKeys: Object.keys(window).filter(
                                (k) => k.includes('MediaSoup') || k.includes('mediasoup')
                            ),
                    ...
                    console.warn(
                        `[Debug Info Failed] Could not gather debug info: ${evalError.message}`
                    );
                }

                console.error(`[Final Retry Failed] Debug info:`, debugInfo);
                throw new Error(
                    `waitForClientWithRetry failed after ${maxRetries} attempts. Last error: ${error.message}`
                );
            }

            console.warn(
                `[Retry ${attempt + 1}/${maxRetries}] Failed: ${error.message}. Retrying...`
            );

            // Brief pause before retry with exponential backoff - but only if page is still alive
            if (!page.isClosed()) {
                const retryDelay = isCI ? 2000 * 1.3 ** attempt : 1000;
...
test.describe('MediaSoup Plugin Loading', () => {
    ...
    test('should initialize plugin successfully', async () => {
            ...
            try {
                if (!page.isClosed()) {
                    finalDebugInfo = await page.evaluate(() => ({
                        ...
                        clientType: window.MediaSoupVTT_Client?.constructor?.name,
                        isInitialized: window.isInitialized,
                        allGlobals: Object.keys(window).filter(
                            (k) => k.startsWith('MediaSoup') || k.includes('mediasoup')
                        ),
                        hookCalls: window.testHookCalls || [],
                    }));
                } else {
        ...
        await expect(page.locator('#test-status')).toContainText('Plugin initialized', {
    ...
    test('should register settings correctly', async () => {
        ...
        const settings = await page.evaluate(() => {
            return window.game.settings
                .getAllSettings()
                .filter((setting) => setting.key.startsWith('mediasoup-vtt.'));
        });
    ...
    test('should register settings menu correctly', async () => {
        ...
        const menus = await page.waitForFunction(
            () => {
                ...
                const allMenus = window.game.settings
                    .getAllMenus()
                    .filter((menu) => menu.key.startsWith('mediasoup-vtt.'));
    ...
    test('should show appropriate notifications', async () => {
        ...
        // Check notifications
        const notifications = await page.evaluate(() => window.ui.notifications.getAll());
    ...
    test('should handle corrupted plugin bundle gracefully', async () => {
        ...
        await expect(page.locator('#test-status')).toContainText('Plugin load failed', {
