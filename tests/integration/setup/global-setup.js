...
function createTestFixtures() {
    ...
    writeFileSync(join(fixturesDir, 'test-data.json'), JSON.stringify(testData, null, 2));

    console.log('[GlobalSetup] Test fixtures created');
}

async function _startMediaSoupServer() {
    ...
    try {
        ...
        mediasoupServer.on('exit', (code, signal) => {
            console.log(
                `[GlobalSetup] MediaSoup server exited with code ${code}, signal ${signal}`
            );
        });
...
async function _waitForServer(url, timeout = 30000) {
