...
export class MediaSoupVTTClient {
  ...
  producerToConsumerMap: Map<string, string>;
  consumerToUserMap: Map<string, string>;
  serverUrl: string;
  isConnected: boolean;
  isConnecting: boolean;
  requestMap: Map<string, SignalingRequestHandlers>;
  requestIdCounter: number;
  localAudioStream: MediaStream | null;
  localVideoStream: MediaStream | null;
  remoteUserStreams: Map<string, any>;
  availableAudioDevices?: Record<string, string>;
  availableVideoDevices?: Record<string, string>;
  ...
  _sendSignalingRequest(requestData: any, timeoutMs = SIGNALING_REQUEST_TIMEOUT_MS): Promise<any> {
  ...
  async pauseProducer(producer: any) {
    ...
    try {
      ...
      if (this.socket && this.socket.readyState === WebSocket.OPEN) {
        ...
        }).catch((e: any) => log(`Error notifying server of producer pause: ${e.message}`, 'warn'));
      }
    } catch (error: any) {
  ...
  async _handleNewRemoteProducer({ producerId, userId, kind, appData }: any) {
  ...
  _handleRemoteProducerClosed(producerId: string) {
  ...
  // UI helper methods
  _updateConnectionStatus(status: string) {
    ...
    if (connectButton) {
      ...
      (connectButton as HTMLElement).title = `MediaSoup: ${status}`;
    }

    log(`Connection status updated: ${status}`, 'debug');
  }

  _updateMediaButtonState(mediaTag: string, isActive: boolean, isPaused: boolean) {
    ...
    if (button) {
      ...
      (button as HTMLElement).title = `${mediaTag}: ${status}`;
  ...
  _displayLocalVideoPreview(stream: MediaStream | null) {
  ...
  _removeRemoteVideoElement(userId: string) {
  ...
  async _populateDeviceSettings() {
    ...
    try {
      ...
      const audioDevices: Record<string, string> = { default: 'Browser Default' };
      const videoDevices: Record<string, string> = { default: 'Browser Default' };
